...
export const ThreadListV2Row = memo(function ThreadListV2Row(props: {
  ...
  const snoozableCardMenuActions = useMemo<MenuAction[]>(
    ...
    [pinMenuItems, snoozePresetActions, titleRegenerationMenuItems],
  );
  const cardMenuActions = useMemo<MenuAction[]>(
    ...
    [pinMenuItems, titleRegenerationMenuItems],
  );
  const slimMenuActions = useMemo<MenuAction[]>(
    () => [
      SLIM_MENU_ACTIONS[0]!,
      ...(thread.pinnedAt != null ? pinMenuItems : []),
      ...titleRegenerationMenuItems,
      SLIM_MENU_ACTIONS[1]!,
    ],
    [pinMenuItems, thread.pinnedAt, titleRegenerationMenuItems],
  );
  ...
  return (
    <>
      <ThreadSwipeable
        ...
        resetKey={`${thread.environmentId}:${thread.id}:${variant}:${snoozedRow}:${thread.settledAt}:${thread.snoozedUntil}`}
