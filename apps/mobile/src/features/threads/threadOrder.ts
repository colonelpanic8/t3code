...
type OrderRow = Pick<
  ...
  "id" | "environmentId" | "pinOrderKey" | "pinnedAt"
>;

type ThreadListOrderSection = "pinned" | "active";
...
function rowOrder(row: OrderRow) {
  return {
    key: row.pinOrderKey ?? null,
    anchor: row.pinnedAt ?? "",
  };
}
...
export function createThreadMovePlanner(input: {
  ...
  readonly section: ThreadListOrderSection;
  readonly reorderableEnvironmentIds: ReadonlySet<EnvironmentId>;
}) {
  if (input.section !== "pinned") {
    return (_movedId: string, _direction: "up" | "down") => null;
  }
...
export function createPendingThreadOrder(input: {
  ...
  return {
    ...
    orderedIds,
    before: new Map(input.ordered.map((row) => [rowId(row), rowOrder(row)])),
...
export function applyPendingThreadOrder<T extends OrderRow>(
  rows: readonly T[],
  section: ThreadListOrderSection,
