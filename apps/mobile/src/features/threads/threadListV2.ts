...
/** Static active order: creation/reopen time changes it; activity does not. */
...
/** Canonical card-section order, independent of search or scope. Only pinned
    rows can carry an optimistic reorder. */
export function getThreadListV2OrderedSection(input: {
  ...
  if (input.section === "active") return ordered;
  const pending =
    input.pendingOrder == null ? null : reconcilePendingThreadOrder(input.pendingOrder, ordered);
  return applyPendingThreadOrder(ordered, "pinned", pending);
}
