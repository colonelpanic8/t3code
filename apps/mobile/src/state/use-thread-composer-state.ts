...
export function useThreadComposerState() {
  ...
  const selectedThreadKey = selectedThreadShell
    ...
    : null;
  // The creation entry is the thread itself, not a follow-up waiting behind it.
  ...
  const selectedThreadFeed = useMemo(() => {
    const feed = buildThreadFeed(selectedThreadVisibleTurnItems, {
      ...(pendingCreationMessage === null
        ? {}
        : { anchoredMessages: [pendingThreadCreationMessage(pendingCreationMessage)] }),
  ...
  const onSendMessage = useCallback(async () => {
    ...
    // A follow-up cannot be delivered until the queued creation has become a
    // real server thread. Avoid stranding it if creation is rejected.
    ...
    enqueuePromise.then(
      ...
      (error: unknown) => {
        ...
        // append so anything typed while the write was in flight wins no files.
