...
it.effect("publishes, rotates, and removes a private loopback advertisement", () =>
  Effect.gen(function* () {
    ...
    const issuedCredentials = [
      {
        id: "discovery-initial-id",
        credential: "DISCOVERY_INITIAL",
        expiresAt: DateTime.makeUnsafe("1970-01-01T00:02:00.000Z"),
      },
      {
        id: "rotated-id",
        credential: "ROTATED",
    ...
    const authLayer = Layer.mock(EnvironmentAuth.EnvironmentAuth)({
      issueStartupPairingCredential: () => Effect.sync(() => issuedCredentials.shift()!),
    ...
    yield* startLocalServerAdvertisement({
      terminalAccessInfo: {
        pairingCredentialId: "terminal-id",
        connectionString: "http://127.0.0.1:3773",
        token: "TERMINAL",
        pairingUrl: "http://127.0.0.1:3773/pair#token=TERMINAL",
        pairingExpiresAt: "1970-01-01T00:05:00.000Z",
      },
      platform: "linux",
    ...
    ).toContain("token=DISCOVERY_INITIAL");
    ...
    expect(revokedIds).toContain("discovery-initial-id");
    expect(revokedIds).not.toContain("terminal-id");
...
it.effect("captures revoke defects and interruptions during cleanup", () =>
  Effect.gen(function* () {
    const fileSystem = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const runtimeDirectory = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "t3-server-advertisement-revoke-test-",
    });
    const baseConfig = yield* Effect.gen(function* () {
      return yield* ServerConfig.ServerConfig;
    }).pipe(Effect.provide(ServerConfig.layerTest(runtimeDirectory, runtimeDirectory)));
    ...
    const revokeFailures = [
      ["defect", Effect.die("revoke defect")],
      ["interruption", Effect.interrupt],
    ] as const;

    for (const [failureKind, revokeFailure] of revokeFailures) {
      const authLayer = Layer.mock(EnvironmentAuth.EnvironmentAuth)({
        issueStartupPairingCredential: () =>
          Effect.succeed({
            id: `${failureKind}-id`,
            credential: failureKind.toUpperCase(),
            expiresAt: DateTime.makeUnsafe("2099-01-01T00:00:00.000Z"),
          }),
        revokePairingLink: () => revokeFailure,
      });
      const advertisementScope = yield* Scope.make();
      ...
      assert.strictEqual(entries.length, 1);
      const recordPath = path.join(directory, entries[0]!);
      const closeExit = yield* Effect.exit(Scope.close(advertisementScope, Exit.void));

      expect(Exit.isSuccess(closeExit), failureKind).toBe(true);
      expect(yield* fileSystem.exists(recordPath)).toBe(false);
    }
  }).pipe(Effect.provide(NodeServices.layer)),
);
