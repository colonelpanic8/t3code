...
// A failed rotation leaves `currentAccessInfo` pointing at an already-expiring
// credential, so the next refresh delay collapses to the minimum. Back off on
// an explicit interval instead of retrying once a second for the life of the
// process.
const FAILED_ROTATION_RETRY_DELAY_MS = 30_000;
...
export const startLocalServerAdvertisement = Effect.fn("server.localAdvertisement.start")(
  function* (input: {
    readonly terminalAccessInfo: HeadlessServeAccessInfo;
    ...
    const port = Number(new URL(input.terminalAccessInfo.connectionString).port);
    ...
    const revoke = Effect.fn("server.localAdvertisement.revoke")(function* (
      accessInfo: HeadlessServeAccessInfo,
    ) {
      const revokeExit = yield* Effect.exit(
        serverAuth.revokePairingLink(accessInfo.pairingCredentialId),
      );
      if (Exit.isFailure(revokeExit)) {
        yield* Effect.logWarning("Could not revoke a local discovery pairing credential.", {
          pairingCredentialId: accessInfo.pairingCredentialId,
          cause: revokeExit.cause,
        });
      }
    });
    ...
    const issueAccessInfo = serverAuth.issueStartupPairingCredential().pipe(
      Effect.map(
        (issued): HeadlessServeAccessInfo => ({
          ...
          connectionString: input.terminalAccessInfo.connectionString,
          token: issued.credential,
          pairingUrl: buildPairingUrl(input.terminalAccessInfo.connectionString, issued.credential),
          pairingExpiresAt: DateTime.formatIso(issued.expiresAt),
        }),
      ),
    );

    const initialAccessInfoExit = yield* Effect.exit(issueAccessInfo);
    if (Exit.isFailure(initialAccessInfoExit)) {
      yield* Effect.logWarning("Local T3 Code server discovery is unavailable.", {
        recordPath,
        cause: initialAccessInfoExit.cause,
      });
      return;
    }
    const initialAccessInfo = initialAccessInfoExit.value;
    const currentAccessInfo = yield* Ref.make(initialAccessInfo);
    ...
    if (Exit.isFailure(initialPublish)) {
      ...
      yield* revoke(initialAccessInfo);
      return;
    }
    ...
    const rotate = Effect.forever(
      Effect.gen(function* () {
        ...
        if (Exit.isFailure(nextExit)) {
          ...
          yield* Effect.sleep(Duration.millis(FAILED_ROTATION_RETRY_DELAY_MS));
          return;
        }
