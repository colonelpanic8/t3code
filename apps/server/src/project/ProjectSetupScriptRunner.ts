...
export const make = Effect.gen(function* () {
  ...
  const runForThread: ProjectSetupScriptRunner["Service"]["runForThread"] = Effect.fn(
    ...
    const projectId = project.id ?? (input.projectId ? ProjectId.make(input.projectId) : null);
    const scripts =
      projectId === null
        ? project.scripts.length > 0
          ? project.scripts
          : settings.defaultProjectScripts
        : resolveProjectScripts(settings, { id: projectId, scripts: project.scripts });
    const script = setupProjectScript(scripts);
    if (!script) {
      return {
        status: "no-script",
      } as const;
