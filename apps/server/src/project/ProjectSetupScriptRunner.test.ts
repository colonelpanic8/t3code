...
it.effect("inherits the machine setup script when the project has no override", () => {
  const open = vi.fn((input: Parameters<TerminalManager.TerminalManager["Service"]["open"]>[0]) =>
    Effect.succeed({
      threadId: input.threadId,
      terminalId: input.terminalId,
  ...
  const projectId = ProjectId.make("project:setup-runner-default");
  const project = {
    id: projectId,
    title: "Project",
    workspaceRoot: "/repo",
  ...
  return Effect.gen(function* () {
    ...
    assert.equal(write.mock.calls[0]?.[0].data, "pnpm install\r");
  }).pipe(Effect.provide(layer));
});
