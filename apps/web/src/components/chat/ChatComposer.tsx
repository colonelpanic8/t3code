...
export interface ChatComposerProps {
  ...
  /**
   * Compaction gating owned by the host surface. Absent means the host has no
   * extra reason to block it, so only the composer's own conditions apply.
   */
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const attachmentBlockReason = supportsAttachmentUploads
    ...
    : null;
  const sendDisabledReason = activePendingProgress ? null : attachmentBlockReason;
  ...
  const resolvedCompactDisabledReason =
    compactDisabledReason ?? (noProviderAvailable ? providerAvailabilityCopy.placeholder : null);
  ...
  const [dismissedTasksRunId, setDismissedTasksRunId] = useState<RunId | null>(null);
  ...
  const activeTasksRunId = activeThread?.latestRun?.runId ?? null;
  const tasksDismissedForActiveRun =
    activeTasksRunId !== null && dismissedTasksRunId === activeTasksRunId;
  const visibleTasksProgress = tasksDismissedForActiveRun ? null : activeTasksProgress;
  const visibleTaskSteps = tasksDismissedForActiveRun ? null : activeTaskSteps;
  const hasBlockingComposerTopDrawer =
    activePendingApproval !== null || pendingUserInputs.length > 0;
  const dismissTasks = useCallback(() => {
    if (activeTasksRunId !== null) {
      setDismissedTasksRunId(activeTasksRunId);
    }
