...
// threads keep the dragged position; active and settled threads use their
// canonical sort order. Snoozed rows can leave the shelf, but dropping into
// it is not supported because snoozing requires a wake time.
...
/** Resolve the destination section and pinned order from an arrayMove across
...
export function planSidebarThreadDrop(input: {
  ...
  switch (target.section) {
    case "active":
      if (activeSection === "active") return { kind: "none" };
      return {
        kind: "activate",
        unpin: activePinned,
...
export function applySidebarThreadDrop<
  T extends Pick<
    ...
    "pinnedAt" | "pinOrderKey" | "snoozedAt" | "snoozedUntil" | "settledAt" | "settledOverride"
