...
describe("sidebar drag projection", () => {
  ...
  it.each([
    ["a1", "a2"],
    ["a2", "a1"],
  ])("keeps canonical Active order while hovering from %s to %s", (active, over) => {
    const items = [
      pinnedHeader,
      thread("p", "pinned"),
      divider,
  ...
  it.each([sidebarMarkerId("pinned-divider"), "a1", "a2"] as const)(
    "opens the canonical Active slot over %s without adding an empty pinned row",
    (over) => {
      const items = [
        pinnedHeader,
        thread("p", "pinned"),
      ...
      expect(result.get("a1")?.y).toBe(0);
      expect(result.get("a2")?.y).toBe(0);
  ...
  it("opens label space below each pinned boundary while dragging", () => {
    ...
    // The boundary labels shift each section without changing Active order.
    const result = preview(
      {
        items,
        activeSortedKeys: ["a1", "a2"],
    ...
    expect(result.get("a1")?.y).toBe(32);
  ...
  it.each(["a1", "a2"] as const)(
    "reserves a full card at the canonical slot over %s when a slim row enters Active",
    (over) => {
      const items = [
        pinnedHeader,
        thread("p", "pinned"),
