 ...
 * card when that panel is open (it owns the top-right corner), otherwise at
 * the frame's normal top-right position.
 */
function initialPreviewMiniPlayerPosition(
  container: HTMLElement,
  frame: PreviewMiniPlayerFrame,
) {
  const fallback = { x: frame.x, y: frame.y };
...
export function ThreadPreviewMiniPlayer({ threadRef, tabId, bottomInset }: Props) {
  ...
  const frame =
    resolvedFrame &&
    container &&
    miniPlayer?.position === null &&
    containerRef.current !== null
      ? {
          ...resolvedFrame,
          ...clampPreviewMiniPlayerPosition(
            initialPreviewMiniPlayerPosition(containerRef.current, resolvedFrame),
            container,
            resolvedFrame,
            bottomInset,
          ),
