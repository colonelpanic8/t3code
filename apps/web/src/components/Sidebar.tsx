...
export default function Sidebar() {
  ...
  const [optimisticDrop, setOptimisticDrop] = useState<{
    ...
    /** Pinned order keys before the drop, to recognize concurrent writes. */
  ...
  useEffect(() => {
    ...
    if (optimisticDrop.order === null) {
      // Lifecycle-only drops can emit several events. Wait for the entire
    ...
    const destinationKeys = pinnedKeys;
  ...
  const draggedSectionOrders = useMemo(() => {
    const thread = dragState === null ? undefined : threadByKey.get(dragState.activeKey);
    if (dragState === null || thread === undefined) return { active: [], settled: [] };
    const key = (candidate: EnvironmentThreadShell) =>
      scopedThreadKey(scopeThreadRef(candidate.environmentId, candidate.id));
    return {
      active: sortThreadsForSidebar([
  ...
  }, [activeThreads, dragState, settledThreads, threadByKey]);
  const sidebarSortingStrategy = useMemo(
    () =>
      createSidebarSortingStrategy({
        items: sidebarListItems,
        boundaryLabelHeight: SIDEBAR_DRAG_LABEL_HEIGHT,
        activeSortedKeys: draggedSectionOrders.active,
        settledOrder: draggedSectionOrders.settled,
    ...
    (event: DragEndEvent) => {
      ...
      const drop = {
        ...
        clearsSnooze:
          plan.kind === "pin" ||
          plan.kind === "settle" ||
          (plan.kind === "activate" && plan.unsnooze),
