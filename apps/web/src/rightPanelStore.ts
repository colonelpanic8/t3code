...
// Every panel action is a user choice unless it goes through `automaticUpdate`.
// Proactive opens and resource reconciliation are automatic.
const automaticUpdate = (
  state: RightPanelStoreState,
  ref: ScopedThreadRef,
  updater: (current: ThreadRightPanelState) => ThreadRightPanelState,
): Partial<RightPanelStoreState> => updateThread(state, ref, updater);

const advanceUserAction = (
  state: RightPanelStoreState,
  ref: ScopedThreadRef,
): Pick<RightPanelStoreState, "userActionRevisionByThreadKey"> => {
  const threadKey = scopedThreadKey(ref);
  return {
