...
const makeHarness = Effect.fn("TestThreadAtoms.makeHarness")(function* (options?: {
  ...
  readonly initialLoad?: Effect.Effect<ThreadSnapshotLoadResult>;
  ...
  const snapshotLoad: ThreadSnapshotLoadResult = options?.httpNone
    ? { _tag: "unavailable" }
    : {
        _tag: "present",
        snapshot,
      ...
      Layer.succeed(
        ...
        ThreadSnapshotLoader.of({
          load: () =>
            Effect.sync(() => {
              httpLoads += 1;
            }).pipe(Effect.andThen(options?.initialLoad ?? Effect.succeed(snapshotLoad))),
        }),
      ),
    ),
  );
  ...
  it.effect.each([
    ...
    ({ kind, httpNone }) =>
      Effect.gen(function* () {
        ...
        yield* Queue.offer(next.events, { kind: "snapshot", ...SNAPSHOT });
    ...
    ({ kind, deleted }) =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
                ...
                Effect.andThen(
                  (
                    deleted
                      ...
                      : Option.getOrNull(value.data)?.thread.title === "Buffer drained"
                  )
          ...
          expect(Option.getOrThrow(final.data).thread.title).toBe("Buffer drained");
