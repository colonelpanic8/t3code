  ...
  const setThread = Effect.fn("EnvironmentThreadState.setThread")(function* (
    ...
    const next = yield* SubscriptionRef.updateAndGet(state, (previous) => {
      ...
      return {
        ...
        // Buffered values from a terminated attempt may still arrive after its
        // diagnostic; only an actual retry may clear that error.
    ...
    const result = yield* SubscriptionRef.modify(
      ...
      (current): readonly [EventApplyResult, EnvironmentThreadState] => {
        ...
        const updated: EnvironmentThreadState = {
          ...
          status: Option.isSome(current.error) ? "cached" : waiting ? "synchronizing" : "live",
          error: current.error,
          history,
        };
