 ...
 */
function threadUserActivityAnchorAt(thread: ThreadActivitySource): string | null {
  const messageAt = thread.latestUserMessageAt;
  const requestedAt = thread.latestTurn?.requestedAt;
  let anchor = thread.createdAt ?? null;
  for (const candidate of [messageAt, requestedAt]) {
    if (candidate != null && (anchor === null || Date.parse(candidate) > Date.parse(anchor))) {
      anchor = candidate;
    }
  }
  return anchor;
...
export function changeRequestAutoSettles(
  ...
  const anchorAt = threadUserActivityAnchorAt(options.thread);
  const anchorAtMs = anchorAt === null ? Number.NaN : Date.parse(anchorAt);
